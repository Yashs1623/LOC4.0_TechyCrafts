...
module.exports.fundRaisePost = async (req, res) => {
  const { title, org_name, price, desc , date } = req.body;
  try {
    const fundRaise = await FundRaise.create({ title, org_name, price, desc , date  });
    // console.log(ngo);
    res.status(201).json(
      {
        success: true,
...
module.exports.createEvent = async(req,res)=>{
  const { event_name, place,  desc , volunteers_required , date } = req.body;
  try {
    const createEvent = await CreateEvent.create({ event_name, place,  desc , volunteers_required , date});
    // console.log(ngo);
    res.status(201).json(
      {
        success: true,
...
module.exports.getEvents = async(req, res) => {
  if (auth.user) {
    var getEvents=[];
    getEvents = await CreateEvent.find({});
